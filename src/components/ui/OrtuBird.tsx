 ...
 * public/app-icon.svg.
 *
 * The source artwork keeps its parts on separate paths, so this is a rig rather
 * than a sprite:
 *
 ...
 * head group can sit on top of the body without changing how the bird looks.
 *
...
/**
 * Two palettes, blended by `tone`.
 *
 * LOGO is the icon's own colours, drawn for a pale sage disc. The bird wears
 * these while it sits in the navbar badge, so it is indistinguishable from the
 * logo it has replaced — and as it lands back in the badge, so the swap is clean.
 *
 * PAGE lifts the greys for flight over #08090c, where #4D4D4D would all but
 * vanish, while keeping their relationships: body darker than wing, upper beak
 * lighter than lower. Feet move to the brand accent (from #FF873D, a hair off).
 */
const LOGO = {
  body: "#4D4D4D",
  wing: "#666666",
...
export interface BirdRig {
  ...
  /** Eyelid, 0 open to 1 shut (scales the lid over the eye). */
  lid: MotionValue<number>;
  /** 0 wears the logo's colours, 1 the lifted colours for the dark page. */
...
const OrtuBird = ({ rig, width, className = "" }: OrtuBirdProps) => {
  const body = useTransform(rig.tone, [0, 1], [LOGO.body, PAGE.body]);
  const wing = useTransform(rig.tone, [0, 1], [LOGO.wing, PAGE.wing]);
  const beakUpper = useTransform(rig.tone, [0, 1], [LOGO.beakUpper, PAGE.beakUpper]);
  const beakLower = useTransform(rig.tone, [0, 1], [LOGO.beakLower, PAGE.beakLower]);
  const foot = useTransform(rig.tone, [0, 1], [LOGO.foot, PAGE.foot]);
  const pupil = useTransform(rig.tone, [0, 1], [LOGO.pupil, PAGE.pupil]);
  ...
  return (
    ...
    >
      ...
      <motion.g style={{ ...fillBox, transformOrigin: HIP_PIVOT, rotate: rig.legTuck }}>
        <motion.path style={{ fill: body }} d="M268.689,298.644l0.34,0.64l13.41,25.567c0,0,1.295,2.984,0.763,4.048 c-0.532,1.063-4.265-0.75-7.605,0.804c-3.339,1.554-4.524,3.666-4.524,3.666s-2.562-0.545-2.657-2.657s-24.504-25.813-24.79-24.804 c-0.273,1.009,25.049-7.251,25.049-7.251L268.689,298.644z" />
        <motion.path style={{ fill: foot }} d="M282.167,328.15c0,0,2.344,4.565,2.344,7.223c0,0.777-0.137,1.09-1.853,1.99l-15.36,5.233 c6.555,6.733,4.157,10.358,0.137,13.615c0.423-6.623-2.276-7.005-5.411-8.981c-1.158,2.317-2.371,3.121-1.608,9.771 s6.364,10.876,6.364,10.876s-5.479,2.808-10.562-2.29c-5.083-5.083-2.808-12.102-2.808-12.102 c-8.082,12.484-1.54,13.178,2.576,18.235c-15.074,1.376-15.223-14.91-10.807-18.589c-4.852,5.629-6.161,12.02-0.464,18.672 c-9.295,1.254-11.748-8.463-6.378-18.317c4.238-7.55,24.831-16.218,33.976-18.916l-2.126-3.707 c0.736-4.062,4.361-5.342,12.007-2.725L282.167,328.15z" />
      </motion.g>
      <motion.g style={{ ...fillBox, transformOrigin: HIP_PIVOT, rotate: rig.legTuck }}>
        <motion.path style={{ fill: body }} d="M305.009,298.644l0.34,0.64l13.424,25.567c0,0,1.295,2.984,0.763,4.048 c-0.532,1.063-4.265-0.75-7.605,0.804c-3.339,1.554-4.524,3.666-4.524,3.666s-2.562-0.545-2.657-2.657s-24.504-25.813-24.79-24.804 c-0.286,1.009,25.049-7.251,25.049-7.251V298.644z" />
        <motion.path style={{ fill: foot }} d="M318.473,328.15c0,0,2.344,4.565,2.344,7.223c0,0.777-0.122,1.09-1.853,1.99l-15.346,5.233 c6.555,6.733,4.157,10.358,0.137,13.615c0.423-6.623-2.276-7.005-5.411-8.981c-1.158,2.317-2.371,3.121-1.608,9.771 c0.763,6.65,6.364,10.876,6.364,10.876s-5.479,2.808-10.563-2.29c-5.083-5.083-2.808-12.102-2.808-12.102 c-8.082,12.484-1.54,13.178,2.576,18.235c-15.074,1.376-15.223-14.91-10.807-18.589c-4.852,5.629-6.161,12.02-0.464,18.672 c-9.295,1.254-11.748-8.463-6.378-18.317c4.238-7.55,24.831-16.218,33.976-18.916l-2.126-3.707c0.75-4.062,4.361-5.342,12.007-2.725 L318.473,328.15z" />
      </motion.g>
      ...
      <motion.g style={{ ...viewBox, transformOrigin: NECK_PIVOT, rotate: rig.headTurn }}>
        <motion.path style={{ fill: beakUpper }} d="M83.939,187.04l-18.371,20.62l29.193-14.732l51.04-14.133l0.682-12.906 c-0.968-0.654-4.02-1.963-2.74-5.125l-59.789,26.263L83.939,187.04z" />
        <motion.path style={{ fill: beakLower }} d="M64.518,207.946l30.228-15.006l51.04-14.133l2.685,9.827L96.6,199.51l-32.082,8.449V207.946z" />
        <motion.path style={{ fill: body }} d="M224.164,161.145l25.363,29.042l-66.808,39.673c-14.324,0.79-19.271-34.331-39.087-42.74 c-8.204-3.475-0.027-3.761-0.027-13.452c0-8.954-2.29-10.712,2.276-16.096c20.688-24.381,66.494-23.345,78.296,3.571 L224.164,161.145L224.164,161.145z" />
        <path fill={EYE} d="M162.263,161.213c4.238,0,7.686,3.434,7.686,7.686c0,4.238-3.434,7.686-7.686,7.686 c-4.238,0-7.686-3.434-7.686-7.686C154.576,164.661,158.01,161.213,162.263,161.213z" />
        <motion.path style={{ fill: pupil }} d="M160.981,165.26c2.14,0,3.857,1.731,3.857,3.857s-1.731,3.857-3.857,3.857s-3.857-1.731-3.857-3.857 S158.856,165.26,160.981,165.26z" />
        ...
        <motion.ellipse
          ...
          rx="8.6"
          ry="8.6"
          style={{ ...fillBox, fill: body, transformOrigin: "50% 50%", scaleY: rig.lid }}
        />
      </motion.g>
    </svg>
  );
